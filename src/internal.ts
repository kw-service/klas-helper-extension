...
export const internalPathFunctions = {
  ...
  '/spv/lis/lctre/viewer/LctreCntntsViewSpvPage.do': () => {
    // 온라인 강의 동영상 다운로드
    const downloadVideo = (videoCode: string) => {
      ...
      browser.runtime.sendMessage({
        ...
        browser.runtime.sendMessage({
          ...
          labelElement.onclick = () => {
            // 한번만 다운로드 가능
            const downloadButton: any = document.getElementsByClassName('download-slide')[0];
              ...
              for (let i = 0; i < slideURIList.length; i++) {
                browser.runtime.sendMessage({
                  ...
                  if (imageBlobs.length === slideURIList.length) {
                    const FileSaverSrc = chrome.runtime.getURL('assets/js/FileSaver.js');
                    const FileSaverLib = await import(FileSaverSrc);
                    // JSZip 로드
                    const JSZipSrc = chrome.runtime.getURL('assets/js/jszip.min.js');
  ...
  '/std/cmn/frame/Frame.do': () => {
    ...
    const setTimeTableIdx = async () => {
      try {
        const selectElement = document.querySelector('.scheduletitle > select');
        const idx = (selectElement as any).selectedIndex;
        await browser.storage.sync.set({ 'timeTableIdx': idx });
        alert(`시간표가 ${(selectElement as any).options[idx].innerHTML}로 고정되었습니다.`);
      }
      catch (e) {
